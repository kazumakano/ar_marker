...
def draw_poses(cam_mat: np.ndarray, cam_dist_coef: np.ndarray, img: np.ndarray, rvecs: np.ndarray | None, tvecs: np.ndarray | None) -> None:
    if rvecs is not None and tvecs is not None:
        for i in range(len(rvecs)):
            cv2.drawFrameAxes(img, cam_mat, cam_dist_coef, rvecs[i], tvecs[i], 0.1)
